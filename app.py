...
from document_processor import DocumentProcessor, get_supported_extensions, is_file_supported, is_url_valid, is_youtube_url
...
def initialize_langchain_manager(model_name: str, temperature: float, 
                                conversation_limit: int = 10, enable_conversation: bool = True) -> LangChainManager:
...
def save_feedback(message_id: str, feedback: str, response_text: str, response_time: float, tools_used: bool):
...
def get_feedback_analytics() -> dict:
    ...
    return {
        ...
        "satisfaction_rate": round((thumbs_up / len(feedback_list)) * 100, 1) if feedback_list else 0
    }
...
def get_system_improvements() -> list:
    ...
    if analytics["thumbs_down"] > analytics["thumbs_up"]:
        improvements.append("🔧 Consider improving response quality - more thumbs down than up")
    ...
    if analytics["satisfaction_rate"] < 70:
        improvements.append("📊 Low satisfaction rate - review tool usage and prompts")
    
    if analytics["total"] < 10:
        improvements.append("📈 Collect more feedback for better insights")
    
    return improvements

def main():
    st.title("🦙 Local Llama Chat with Tools")
    ...
    with st.sidebar:
        ...
        if use_tools:
            ...
            if selected_categories:
                ...
                st.info(f"🔧 {len(tools)} tools selected from {len(selected_categories)} categories")
        ...
        if enable_conversation:
            ...
            if "langchain_manager" in st.session_state and st.session_state.langchain_manager:
                msg_count = len(st.session_state.langchain_manager.conversation_history)
                st.info(f"📝 Current conversation: {msg_count}/{conversation_limit} messages")
        ...
        # Metal GPU status
        st.subheader("🍎 Metal GPU Status")
        try:
            # Check if Metal is being used
            models = ollama.list()
            if hasattr(models, 'models') and models.models:
                st.success("✅ Metal GPU acceleration available")
                ...
                st.info("🔄 Pull a model to enable Metal acceleration")
        except:
        ...
        if analytics["total"] > 0:
            ...
            with col1:
                st.metric("👍 Thumbs Up", analytics["thumbs_up"])
            with col2:
                st.metric("👎 Thumbs Down", analytics["thumbs_down"])
            ...
            if improvements:
                with st.expander("🔧 System Improvement Suggestions"):
        ...
        st.subheader("📄 Enhanced Document Processing")
        ...
        if st.session_state.document_processor is None and selected_model:
            ...
            st.info("🚀 Enhanced document processor initialized")
        
        processor = st.session_state.document_processor
        
        # Tab interface for different input types
        tab1, tab2, tab3, tab4 = st.tabs(["📁 File Upload", "🌐 Web Content", "🗃️ Database Query", "🎥 YouTube Video"])
                ...
                if st.button("📁 Process Documents", key="process_docs"):
                    with st.spinner("Processing documents..."):
                        ...
                        if success_count > 0:
                            ...
                            if (st.session_state.langchain_manager and 
                                st.session_state.langchain_manager.llm and
                                processor.vector_store):
                                if processor.setup_qa_chain(st.session_state.langchain_manager.llm):
                                    st.success("🔍 Document Q&A is ready!")
                                else:
                                    st.warning("⚠️ Documents processed but QA chain setup failed")
                            else:
                                st.warning("⚠️ Documents processed but LLM not ready. Initialize model first.")
                        else:
                            st.error("❌ No documents were processed successfully")
        
        with tab2:
            # Web content processing
            st.write("🌐 Add web content for Q&A")
            ...
            if st.button("🌐 Process Web Content", key="process_web") and url_input:
                    ...
                    with st.spinner("Processing web content..."):
                        if processor.process_web_url(url_input, loader_type):
                            ...
                            if (st.session_state.langchain_manager and 
                                ...
                                processor.vector_store):
                                if processor.setup_qa_chain(st.session_state.langchain_manager.llm):
                                    st.success("🔍 Web content Q&A is ready!")
                                else:
                                    st.warning("⚠️ Web content processed but QA chain setup failed")
                        else:
        ...
        with tab3:
            ...
            st.write("🗃️ Query database for Q&A")
            ...
            if processor.db_connection:
                ...
                if st.button("🗃️ Process Database Query", key="process_db") and sql_query:
                        ...
                        with st.spinner("Processing database query..."):
                            if processor.process_database_query(sql_query):
                                ...
                                if (st.session_state.langchain_manager and 
                                    ...
                                    processor.vector_store):
                                    if processor.setup_qa_chain(st.session_state.langchain_manager.llm):
                                        st.success("🔍 Database Q&A is ready!")
                                    else:
                                        st.warning("⚠️ Database content processed but QA chain setup failed")
                            else:
        ...
        with tab4:
            ...
            st.write("🎥 Add YouTube video transcripts for Q&A")
                    ...
                    if success:
                        ...
                        with st.spinner("Setting up Q&A chain..."):
                            if (st.session_state.langchain_manager and 
                                ...
                                processor.vector_store):
                                if processor.setup_qa_chain(st.session_state.langchain_manager.llm):
                                    st.success("🔍 YouTube video Q&A is ready!")
                                else:
                                    st.warning("⚠️ YouTube content processed but QA chain setup failed")
                            else:
                                st.warning("⚠️ YouTube content processed but LLM not ready")
                    else:
        ...
        if processor:
            ...
            if content_info["total_files"] > 0 or content_info["total_urls"] > 0 or content_info["total_tables"] > 0 or content_info["total_videos"] > 0:
                st.write("**📊 Processed Content:**")
                
                # Files section
                if content_info["total_files"] > 0:
                    st.write(f"📁 **Files ({content_info['total_files']}):**")
                ...
                if content_info["total_urls"] > 0:
                    st.write(f"🌐 **Web Content ({content_info['total_urls']}):**")
                    for url_info in content_info["urls"]:
                        with st.expander(f"🌐 {url_info['url']}"):
                            col1, col2, col3 = st.columns(3)
                            with col1:
                ...
                if content_info["total_tables"] > 0:
                    st.write(f"🗃️ **Database Queries ({content_info['total_tables']}):**")
                    for table_info in content_info["tables"]:
                        with st.expander(f"🗃️ {table_info['query'][:50]}..."):
                            col1, col2, col3 = st.columns(3)
                            with col1:
                ...
                if content_info["total_videos"] > 0:
                    st.write(f"🎥 **YouTube Videos ({content_info['total_videos']}):**")
                    for video_info in content_info["videos"]:
                        ...
                        with st.expander(f"🎥 {video_title}"):
                            ...
                            if video_info.get('view_count'):
                                st.write(f"👁️ Views: {video_info['view_count']:,}")
                ...
                # Summary
                st.metric("📊 Total Chunks", content_info["total_chunks"])
                
                # Clear content button
                if st.button("🗑️ Clear All Content", key="clear_all_content"):
        ...
        if processor and processor.is_ready():
            ...
            st.subheader("📄 Document Q&A Mode")
            doc_qa_mode = st.checkbox(
                "📄 Force Document Q&A (answers only from uploaded content)",
                key="doc_qa_mode",
            ...
            if doc_qa_mode:
                st.info("🔍 Document Q&A mode is ON - Answers will come from uploaded content only")
        elif processor:
            st.warning("⚠️ Content processed but Q&A not ready. Check LLM initialization.")
    
    # Initialize LangChain manager when model or settings change
    if selected_model and ("langchain_manager" not in st.session_state or 
                          st.session_state.langchain_manager is None or 
                          st.session_state.langchain_manager.model_name != selected_model or
                          st.session_state.langchain_manager.temperature != temperature or
                          st.session_state.langchain_manager.conversation_limit != conversation_limit or
                          st.session_state.langchain_manager.enable_conversation != enable_conversation):
        ...
        with st.spinner("Initializing LangChain..."):
            ...
            if use_tools and selected_categories and "langchain_manager" in st.session_state and st.session_state.langchain_manager:
                tools = get_selected_tools(selected_categories)
                st.session_state.langchain_manager.set_tools(tools)
            
            # Setup document QA chain if documents are processed
            if (st.session_state.document_processor and 
                st.session_state.document_processor.vector_store and 
                st.session_state.langchain_manager and 
                st.session_state.langchain_manager.llm):
                if st.session_state.document_processor.setup_qa_chain(st.session_state.langchain_manager.llm):
                    st.success("🔍 Document Q&A chain updated!")
                else:
    ...
    for i, message in enumerate(st.session_state.messages):
        with st.chat_message(message["role"]):
            ...
            if "tools_used" in message:
                st.caption(f"🔧 Tools: {message['tools_used']}")
            ...
            if message["role"] == "assistant" and "feedback_given" not in message:
                ...
                feedback = st.feedback(
                    ...
                    on_change=lambda fid=message_id, txt=message["content"], rt=message.get("response_time", 0), tu="tools_used" in message: save_feedback(fid, "up" if st.session_state[f"feedback_{fid}"] == 1 else "down", txt, rt, tu)
                )
                ...
                feedback_emoji = "👍" if message["feedback_given"] == "up" else "👎"
    ...
    if prompt := st.chat_input("Type your message here..."):
        ...
        if "langchain_manager" not in st.session_state or not st.session_state.langchain_manager:
        ...
        if processor and processor.is_ready():
            ...
            if st.session_state.get("doc_qa_mode", False):
                ...
                st.info("🔍 Using document Q&A (forced mode)...")
            else:
                # Check if the question is about documents (automatic detection)
                doc_keywords = ["what", "how", "who", "when", "where", "tell me about", "find", "search"]
                prompt_lower = prompt.lower()
                if any(keyword in prompt_lower for keyword in doc_keywords):
                    use_document_qa = True
                    st.info("🔍 Using document Q&A...")
        ...
        if use_document_qa:
            ...
            if doc_result["success"]:
                ...
                if doc_result.get("source_documents"):
                    context_preview = "\n\n**Retrieved Context:**\n"
                    for i, doc in enumerate(doc_result["source_documents"][:2]):  # Show first 2 docs
                        source = doc.metadata.get('source', 'Unknown')
                        content_preview = doc.page_content[:150]
                        context_preview += f"Doc {i+1} ({source}): {content_preview}...\n"
                
                response_text = f"📄 **Document Answer:**\n\n{doc_result['answer']}{context_preview}"
                response_time = 2.0
            else:
                # Fall back to regular response
                response_text, response_time = st.session_state.langchain_manager.generate_response(
        ...
        if response_text:
            ...
            if use_tools and selected_categories:
                message_data["tools_used"] = f"{len(get_selected_tools(selected_categories))} tools"
            ...
            with st.chat_message("assistant"):
                ...
                if use_tools and selected_categories:
                    st.caption(f"🔧 Tools: {len(get_selected_tools(selected_categories))} tools")
    
    # Footer
    st.markdown("---")
    st.markdown("💡 **User Tip:**")
    st.markdown("- Use smaller models (like `llama3.2:1b`) for better performance on 8GB RAM, This tool was built for example purposes")
