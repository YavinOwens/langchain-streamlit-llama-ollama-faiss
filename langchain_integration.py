...
try:
    ...
    try:
        ...
        raise ImportError(
            "Neither langchain_ollama nor langchain_community found. Please install: pip install langchain-ollama")
...
class LangChainManager:
    ...
    def generate_response(self, message: str, use_tools: bool = True) -> tuple[str, float]:
        ...
        try:
            ...
            import datetime
            current_context = f"Current date: {
                datetime.date.today().strftime('%Y-%m-%d')}, Current time: {
    ...
    def _handle_direct_tool_call(self, message: str) -> str:
            ...
            for tool in self.tools:
                if tool.name == "web_search":
                    try:
                        ...
                        return f"[SEARCH] Web search results for '{query}':\n{result}"
                    except BaseException:
                        return f"[SEARCH] Search failed for: {query}"
                ...
                for tool in self.tools:
                    if tool.name == "get_weather":
                        try:
                            ...
                            return f"[WEATHER] Weather in {location}:\n{result}"
                        except BaseException:
                            return f"[WEATHER] Weather search failed for: {location}"
        ...
        if ("current time" in message_lower or "what time" in message_lower or
            ...
            return f"[TIME] Current time: {now.strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        if ("current date" in message_lower or "what date" in message_lower or
            ...
            return f"[DATE] Today's date is {today.strftime('%A, %B %d, %Y')}"
        ...
        if ("tomorrow" in message_lower or "date tomorrow" in message_lower or
            ...
            return f"[DATE] Tomorrow's date is {tomorrow.strftime('%A, %B %d, %Y')}"
        ...
        if ("yesterday" in message_lower or "date yesterday" in message_lower or
            ...
            return f"[DATE] Yesterday's date was {yesterday.strftime('%A, %B %d, %Y')}"
