...
class DocumentProcessor:
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            if database_url:
                ...
                st.info("📄 No DATABASE_URL found - database features disabled")
        except Exception as e:
    ...
    def load_document(self, file_path: str, file_type: str) -> List[Document]:
        ...
        try:
            ...
            st.success(f"✅ Loaded {len(documents)} document(s) from {file_type}")
            return documents
        except Exception as e:
            st.error(f"Error loading {file_type} file: {e}")
            return []
    
    def load_web_content(self, url: str, loader_type: str = "web_base") -> List[Document]:
        ...
        try:
            ...
            documents = loader.load()
            st.success(f"✅ Loaded {len(documents)} document(s) from {url}")
            return documents
        except Exception as e:
            st.error(f"Error loading web content from {url}: {e}")
            return []
    
    def load_youtube_content(self, video_url: str, language: str = "en") -> List[Document]:
        """Load transcript from YouTube video using youtube-transcript-api directly"""
        if not YOUTUBE_AVAILABLE:
            st.error("YouTube libraries not available. Please install youtube-transcript-api and yt-dlp")
            return []
        
        try:
            # Extract video ID from URL
            import re
            video_id_match = re.search(r'(?:youtube\.com/watch\?v=|youtu\.be/|youtube\.com/embed/)([a-zA-Z0-9_-]{11})', video_url)
            ...
            try:
                ...
                content += f"Duration: {video_info['duration']} seconds ({video_info['duration']//60}:{video_info['duration']%60:02d})\n"
                ...
                st.success(f"✅ Loaded {transcript_source} transcript from YouTube video: {video_info['title']}")
    ...
    def query_database_to_documents(self, query: str, limit: int = 100) -> List[Document]:
        ...
        try:
            ...
            summary_doc = Document(
                ...
                metadata={"source": "database_query", "query": query, "type": "summary"}
            )
            ...
            if len(df) <= 20:
                for idx, row in df.iterrows():
                    ...
                    doc = Document(
                        page_content=content,
                        metadata={"source": "database_query", "query": query, "row": idx, "type": "row"}
                    )
                ...
                for i in range(0, len(df), chunk_size):
                    ...
                    doc = Document(
                        page_content=content,
                        metadata={"source": "database_query", "query": query, "chunk": i//chunk_size, "type": "chunk"}
                    )
                    documents.append(doc)
            
            st.success(f"✅ Created {len(documents)} documents from {len(df)} database rows")
            return documents
    ...
    def process_database_query(self, query: str) -> bool:
        ...
        if not query or not self.embeddings or not self.db_connection:
            st.error(f"Cannot process query: query={bool(query)}, embeddings={bool(self.embeddings)}, db={bool(self.db_connection)}")
            return False
        
        try:
            st.info(f"🗃️ Processing database query...")
            ...
            st.info(f"📋 Created {len(documents)} document(s) from database")
            ...
            if self.vector_store is None:
                st.info("🏗️ Creating new vector store...")
                ...
                try:
                    ...
                    success = True
                    st.success("✅ Added database content to vector store")
                except Exception as e:
            ...
            if success:
                ...
                st.success(f"✅ Successfully processed database query")
            else:
    ...
    def _create_simple_qa_chain(self, llm):
        ...
        def simple_qa(question):
            try:
                ...
                docs = self.vector_store.similarity_search(question, k=10)  # Increased from 5 to 10
                if not docs:
                    return {"answer": "No relevant information found in documents.", "success": False}
                ...
                for i, doc in enumerate(docs):
                    ...
                    context_parts.append(f"Document {i+1} (Source: {source}): {content}")
...
Answer:"""
            ...
            if callable(self.qa_chain):
                ...
                if isinstance(result, dict):
                    ...
                    if result.get("source_documents"):
                        ...
                        st.info(f"🔍 Retrieved {len(result['source_documents'])} documents from {len(sources)} sources")
                ...
                if isinstance(result, dict):
                    ...
                    if result.get("documents"):
                        sources = set(doc.metadata.get('source', 'Unknown') for doc in result['documents'])
                        st.info(f"🔍 Retrieved {len(result['documents'])} documents from {len(sources)} sources")
    ...
    def process_uploaded_file(self, uploaded_file) -> bool:
        ...
        if not uploaded_file or not self.embeddings:
            st.error(f"Cannot process file: uploaded_file={bool(uploaded_file)}, embeddings={bool(self.embeddings)}")
            return False
    ...
    def process_web_url(self, url: str, loader_type: str = "web_base") -> bool:
        ...
        if not url or not self.embeddings:
            st.error(f"Cannot process URL: url={bool(url)}, embeddings={bool(self.embeddings)}")
            return False
        
        try:
            st.info(f"🌐 Processing web content from {url}...")
            ...
            st.info(f"📋 Loaded {len(documents)} document(s) from web")
            ...
            if self.vector_store is None:
                ...
                try:
                    self.vector_store.add_documents(chunks)
                    success = True
                    st.success("✅ Added web content to vector store")
                except Exception as e:
            ...
            if success:
                ...
                st.success(f"✅ Successfully processed web content from {url}")
            else:
    ...
    def process_youtube_video(self, video_url: str, language: str = "en") -> bool:
        ...
        if not video_url or not self.embeddings:
            st.error(f"Cannot process video: video_url={bool(video_url)}, embeddings={bool(self.embeddings)}")
            return False
        ...
        try:
            ...
            if success:
                ...
                video_info = {"url": video_url, "language": language, "chunks": len(chunks), "documents": len(documents)}
                if documents and documents[0].metadata:
                    metadata = documents[0].metadata
                    video_info.update({
                        "title": metadata.get("title", "Unknown"),
                        "description": metadata.get("description", "")[:100] + "..." if metadata.get("description") else "",
